...
仓库内 `v1.8.1` 热修复候选（2026-08-27 登记，`docs/23` HF-1~4，已获项目负责人发布授权，待发布）：修复自 `v1.7.0` 起真实完整 `cmr update` 全平台必然失败的回归——SHA256SUMS 条目查找误用 npm 版本化 tarball 文件名（`claude-model-router-<version>.tgz`），应为固定资产名（`claude-model-router.tgz`）；`--check` 不走该校验故历次发布门禁未拦截。HF-4：v1.8.1 起 SHA256SUMS 常设第二条同摘要别名条目（npm 落盘名），存量 `1.7.0`–`1.8.0` 用户 `cmr update` 一次即自愈，免手动重装；发布门禁已补「旧版→新版真实完整 `cmr update`」回读（docs/23 §5/§6）。
