...
仓库当前为未发布的 `1.8.2` 候选（`docs/24` DCP-1~4）：两个 DeepSeek Profile 统一注入 `CLAUDE_CODE_MAX_CONTEXT_TOKENS=1048576`，Pricing 刷新为 Pro/Flash/Flash Vision 三模型的工作日峰谷 USD 价格；模型映射、Provider、Secret 与鉴权边界不变。公开 Latest、安装 URL 与 tag 仍为 `v1.8.1`，尚未发布。
