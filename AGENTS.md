...
当前稳定版为 `2.0.0`（2026-09-10 发布，Latest，tag `v2.0.0` 指向门禁 commit `2f4ce292085a76255b81e02a2b2fb6b48459c54e`）：`docs/25` DS41-1~5 DeepSeek V4.1 Flash 破坏性迁移——唯一 `deepseek` Profile 全面迁移到官方模型 `deepseek-flash`，删除 `deepseek-vision` Profile 与 `deepseek-auto`/`deepseek-flash-vision` 旧品牌别名，只保留规范入口 `deepseek` 和通用兼容别名 `build`。Provider、Secret 与鉴权边界不变；Windows T4 run 34472718163 在 Node 18.20.8/24.20.0 双档全绿；固定资产 SHA-256 为 `49cd63654634c8995721df5e46bb840788bde0ac7289a121ee3fe71562921cdf`。immutable Release、exact/latest 字节一致、隔离安装、`update --check` 与 `1.8.2 -> 2.0.0` 真实完整更新均已闭环。

前一稳定版为 `1.8.2`（2026-08-29 发布，tag `v1.8.2` 指向门禁 commit `30cf53e`）：`docs/24` DCP-1~4 DeepSeek 1M 上下文与峰谷 Pricing 刷新——两个 DeepSeek Profile 统一声明 `CLAUDE_CODE_MAX_CONTEXT_TOKENS=1048576`，模型映射不变；Pricing 更新为 Pro/Flash/Flash Vision 三模型的工作日峰谷 USD 价格。Windows T4 run 33244442385 在 Node 18.20.8/24 双档全绿；固定资产 SHA-256 为 `067ec2bf983c03e4692216417de46f17c2cd58065cad1006debd9ae02d7c1178`；exact/latest、隔离安装、`update --check` 与 `1.8.1 -> 1.8.2` 真实完整更新均已闭环。
...
当前仓库与公开稳定版均为 `2.0.0`；release code 与 immutable tag 固定在门禁 commit `2f4ce292085a76255b81e02a2b2fb6b48459c54e`，发布后的证据文档提交只更新状态与回读记录，不移动 tag。
...
未发布候选登记：无。原 `2.0.0` 候选（`docs/25` DS41-1~5，2026-09-10 登记）已于同日经项目负责人授权公开发布并完成全部门禁；真实 DeepSeek 图片请求未执行，也未作为 Release PASS 的依据。原 `1.8.2` 候选（`docs/24` DCP-1~4，2026-08-29 登记）已于同日经项目负责人授权公开发布并完成全部门禁；原 `1.8.1` 热修复候选（`docs/23` HF-1~4，2026-08-27 登记）已于同日经项目负责人授权随 `1.8.1` 公开发布并完成全部门禁（含 HF-3 自愈门禁）；原 `1.8.0` 候选（`docs/22` GFA-1~6，2026-08-26 登记）已于 2026-08-27 随 `1.8.0` 公开发布并完成全部门禁。
