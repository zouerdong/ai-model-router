...
当前仓库与公开稳定版均为 `1.8.2`；发布后的证据文档提交只更新状态与回读记录，不移动 immutable tag。
...
`1.8.2` 提供八个数据化 Profile（本版不新增 Profile）：

- `kimi`：Kimi K3 的完整 Claude Code 模型映射。
- `deepseek`：DeepSeek Auto；主会话由 V4 Pro 承担，Haiku 档与子 Agent 由 V4 Flash Vision 实验模型（`deepseek-v4-flash-vision-exp`，多模态）承担；统一声明 1,048,576 token 客户端窗口。
- `deepseek-vision`：DeepSeek V4 Flash Vision；全部模型映射使用 `deepseek-v4-flash-vision-exp`，复用 `deepseek` Provider 与 Secret；统一声明 1,048,576 token 客户端窗口。
...
未发布候选登记：无。原 `1.8.2` 候选（`docs/24` DCP-1~4，2026-08-29 登记）已于同日经项目负责人授权公开发布并完成全部门禁；原 `1.8.1` 热修复候选（`docs/23` HF-1~4，2026-08-27 登记）已于同日经项目负责人授权随 `1.8.1` 公开发布并完成全部门禁（含 HF-3 自愈门禁）；原 `1.8.0` 候选（`docs/22` GFA-1~6，2026-08-26 登记）已于 2026-08-27 随 `1.8.0` 公开发布并完成全部门禁。
