...
test("launchProfile injects the selected profile and passes Claude args unchanged", async (t) => {
  ...
  assert.equal(glm.snapshot.haiku, "glm-5.3-flash[1m]");
  assert.equal(glm.snapshot.subagent, "glm-5.3-flash[1m]");
  ...
  assert.equal(glmApi.snapshot.opus, "glm-5.3[1m]");
  assert.equal(glmApi.snapshot.sonnet, "glm-5.3[1m]");
  assert.equal(glmApi.snapshot.haiku, "glm-5.3-flash[1m]");
  assert.equal(glmApi.snapshot.subagent, "glm-5.3-flash[1m]");
  ...
  assert.match(glmApi.output, /GLM-5\.3 API \(Pay-as-you-go\) uses direct standard API billing/);
  assert.match(glmApi.output, /GLM-5\.3 tokens: input 8, output 28, cache hit 2; GLM-5\.3-Flash: input 0\.8, output 2\.8, cache hit 0\.23/);
  assert.match(glmApi.output, /verified 2026-08-26/);
  // docs/22 GFA-4: no GLM-5.2 reference, no generic caveat, no promotional prices in the notice.
  assert.doesNotMatch(glmApi.output, /GLM-5\.2/);
  assert.doesNotMatch(glmApi.output, /other mapped models/);
  assert.doesNotMatch(glmApi.output, /0\.115|input 0\.4|output 1\.4/);
...
test("GLM standard API launch clears both authentication modes without mutating the parent", async (t) => {
  ...
  assert.equal(snapshot.opus, "glm-5.3[1m]");
  assert.equal(snapshot.sonnet, "glm-5.3[1m]");
  assert.equal(snapshot.haiku, "glm-5.3-flash[1m]");
  assert.equal(snapshot.subagent, "glm-5.3-flash[1m]");
  ...
  for (const key of ["model", "fable", "effort", "toolSearch"]) assert.equal(snapshot[key], null, key);
